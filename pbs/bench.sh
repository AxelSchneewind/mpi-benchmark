...
make_bench_args() {
	...
	local BENCH_ARGS="--bench-name "$BENCH_NAME" -b $BENCH_BUFFER_SIZE -i $BENCH_ITERATION_COUNT -m $BENCH_MODES -t $BENCH_MIN_THREAD_COUNTS -T $BENCH_MAX_THREAD_COUNTS -p $BENCH_MIN_PARTITION_SIZES -P $BENCH_MAX_PARTITION_SIZES -s $BENCH_SEND_PATTERNS -o $BENCH_OUTPUT_FILES $BENCH_DIFFERENT_PARTITIONINGS"
	echo "$BENCH_ARGS"
}

make_bench_partitionings_args() {
	local BENCH_NAME="partitioned_detailed"
	local BENCH_OUTPUT_FILES="R0_partitionings.csv,R1_partitionings.csv"
	local BENCH_BUFFER_SIZE=23
	local BENCH_ITERATION_COUNT=100
	local BENCH_MODES=Psend
	local BENCH_MIN_THREAD_COUNTS=0
	local BENCH_MAX_THREAD_COUNTS=4
	local BENCH_MIN_PARTITION_SIZES=9
	local BENCH_MAX_PARTITION_SIZES=23
...
make_trace_args() {
	...
	local TRACE_ARGS="-b $TRACE_BUFFER_SIZE -i $TRACE_ITERATION_COUNT -m $TRACE_MODES -t $TRACE_MIN_THREAD_COUNTS -T $TRACE_MAX_THREAD_COUNTS -p $TRACE_MIN_PARTITION_SIZES -P $TRACE_MAX_PARTITION_SIZES -s $TRACE_SEND_PATTERNS $TRACE_DIFFERENT_PARTITIONINGS"
	echo "$TRACE_ARGS"
}
