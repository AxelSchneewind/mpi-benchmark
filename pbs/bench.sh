...
make_bench_args() {
	...
	local BENCH_MODES=Send,SendPersistent,Isend,Psend,IsendTest,PsendParrived
	...
	local BENCH_ARGS="-b $BENCH_BUFFER_SIZE -i $BENCH_ITERATION_COUNT -m $BENCH_MODES -t $BENCH_MIN_THREAD_COUNTS -T $BENCH_MAX_THREAD_COUNTS -p $BENCH_MIN_PARTITION_SIZES -P $BENCH_MAX_PARTITION_SIZES -s $BENCH_SEND_PATTERNS -o $BENCH_OUTPUT_FILES $BENCH_DIFFERENT_PARTITIONINGS"
	echo "$BENCH_ARGS"
}
