...
mode_names = ['Send', 'Isend', 'IsendTest', 'IsendThenTest', 'IsendTestall', 'Psend', 'Psend progress', 'PsendThreaded', 'custom Psend', 'WinSingle', 'Win']
...
def plot(data, ax, mode, columns, domain=None, ylabel=None):
    ...
    for pattern in data['send_pattern'].unique():
        plot_data = mode_data[mode_data['send_pattern'] == pattern]
        for column in columns:
            ...
            ax.plot(xValues, yValues, label=mode_names[mode] + ', ' + pattern)
...
def plot_bandwiths(data):
    ...
    plot(data, ax[3,2], 9, column_names, domain, ylabel=ylabel)
    plt.show()
