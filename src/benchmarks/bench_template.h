...
#define execute(testcase, result, rank, functions) {                                                                                        \
    ...
    for(int it = 0; it < testcase->warmup_iterations; it++) {                                                                               \
        ...
        if (0 == rank) {                                                                                                                    \
            if (NULL != functions.partition_operation_send) {                                                                               \
                _Pragma("omp parallel for num_threads(thread_count)")                                                          \
    ...
    {                                                                                                                                       \
        ...
        timers_stop(timers, IterationStartToWait);                                                                                          \
                                                                                                                                            \
        if (testcase->pre_complete_sleep) {                                                                                                 \
...
#define execute_gpu(testcase, result, rank, functions) {                                                                                \
    char state_buf[functions.state_size];     /* allocate state on the stack*/                                                          \
    void* state = (void*) state_buf;                                                                                                    \
    int thread_count = testcase->thread_count;                                                                                              \
                                                                                                                                        \
