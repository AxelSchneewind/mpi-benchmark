...
static int send_persistent_init(TestCase *test_case, Result *result, int comm_rank, void* s) {
...
static int send_persistent_cleanup(TestCase *test_case, Result *result, int comm_rank, void* s) {
    struct send_bench_state* state = (struct send_bench_state*) s;
    for (int i = 0; i < test_case->partition_count; i++) {
...
static int send_persistent_start(TestCase *test_case, Result *result, int comm_rank, void* s) {
    return 0;
}


static int send_persistent_complete(TestCase *test_case, Result *result, int comm_rank, void* s) {
    struct send_bench_state* state = (struct send_bench_state*) s;
    MPI_CHECK(MPI_Waitall(test_case->partition_count, state->requests, MPI_STATUSES_IGNORE));
    return 0;
}

static int send_persistent_recv_partition_operation(TestCase *test_case, Result *result, int comm_rank, int partition, int threadnum, void* s) {
    struct send_bench_state* state = (struct send_bench_state*) s;
    MPI_CHECK(MPI_Start(&state->requests[partition]));
    return 0;
}

static int send_persistent_send_partition_operation(TestCase *test_case, Result *result, int comm_rank, int partition, int threadnum, void* s) {
    struct send_bench_state* state = (struct send_bench_state*) s;
    assert (0 == comm_rank);
    MPI_CHECK(MPI_Start(&state->requests[partition]));
    return 0;
...
static int send_persistent_test_send_partition_operation(TestCase *test_case, Result *result, int comm_rank, int partition, int threadnum, void* s) {
    struct send_bench_state* state = (struct send_bench_state*) s;
    assert (0 == comm_rank);
    int flag;
    MPI_CHECK(MPI_Start(&state->requests[partition]));
...
static int send_send_partition_operation(TestCase *test_case, Result *result, int comm_rank, int partition, int threadnum, void* s) {
    ...
    MPI_CHECK(MPI_Send(test_case->buffer + test_case->partition_size * partition, test_case->partition_size, MPI_BYTE, 1, partition, MPI_COMM_WORLD));
    return 0;
}
...
static int send_recv_partition_operation(TestCase *test_case, Result *result, int comm_rank, int partition, int threadnum, void* s) {
    ...
    MPI_CHECK(MPI_Recv(test_case->buffer + test_case->partition_size * partition, test_case->partition_size, MPI_BYTE, 0, partition, MPI_COMM_WORLD, &result->recv_status));
    return 0;
}

static const struct benchmarking_function send_persistent_test = {
    .state_size = sizeof(struct send_bench_state),
    .init = &send_persistent_init,
    .start = &send_persistent_start,
    .partition_operation_send = &send_persistent_test_send_partition_operation,
    .partition_operation_recv = &send_persistent_recv_partition_operation,
    .complete = &send_persistent_complete,
    .cleanup = &send_persistent_cleanup
};
...
static const struct benchmarking_function send = {
    .state_size = sizeof(char),
    .init = NULL,
    .start = NULL,
...
{ 
    execute(test_case, result, comm_rank, send);
}
