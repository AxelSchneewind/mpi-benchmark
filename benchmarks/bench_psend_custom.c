...
void bench_custom_psend(TestCase *test_case, Result *result, int comm_rank)
{
	...
	{
		MPI_CHECK(custom_MPI_Precv_init(test_case->buffer, test_case->buffer_size / test_case->partition_size, test_case->partition_size, MPI_CHAR, 0, 0, MPI_COMM_WORLD, MPI_INFO_ENV, &request));
	}
